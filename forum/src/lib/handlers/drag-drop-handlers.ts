...
import { EditorActions, type EditorAction } from '../reducer/actions';
import { isContainerNode, isTextNode} from '../utils/editor';
import { ContainerNode, TextNode, EditorNode } from '../types/editor';
 ...
 * Handle drag over
 */
export function createHandleDragOver(params: Omit<DragDropHandlerParams, 'setIsUploading' | 'onUploadImage'>) {
    ...
    if (draggedNodeId && draggingNodeId) {
      ...
      if (dropPosition === "left" || dropPosition === "right") {
        ...
        if (
          ...
          dispatch(
            EditorActions.updateNode(
              parent.id,
              {
                children: newChildren as EditorNode[],
      ...
      if (draggingResult.parentId && draggingResult.parent) {
        ...
        const remainingChildren = parent.children.filter(
          (c: EditorNode) => c.id !== draggingNodeId
        );
        ...
        if (remainingChildren.length === 1) {
          ...
          actions.push(
            EditorActions.updateNode(
              parent.id,
              {
                children: remainingChildren as EditorNode[],
              } as Partial<ContainerNode>
            )
          );
