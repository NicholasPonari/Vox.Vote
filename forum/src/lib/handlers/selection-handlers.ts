...
import { ContainerNode, SelectionInfo, TextNode, EditorState } from "../types/editor";
import { isTextNode } from "../utils/editor";
import {
...
export function createHandleFormat(params: SelectionHandlerParams) {
  return (format: "bold" | "italic" | "underline") => {
    ...
    setTimeout(() => {
      void container.children.find((n) => n.id === nodeId);
    }, 100);
