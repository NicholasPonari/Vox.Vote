...
import { ContainerNode, TextNode } from '../types/editor';
...
export function createHandleGroupSelectedImages(
  ...
  return () => {
    ...
    if (imageNodes.length < 2) {
      console.error("Could not find all selected images");
      return;
    }
...
export function createHandleReverseImagesInFlex(
  ...
  return () => {
    ...
    console.log(`Order of ${selectedImageIds.size} images reversed`);
  };
}
