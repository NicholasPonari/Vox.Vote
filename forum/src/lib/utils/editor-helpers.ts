...
import { TextNode, EditorNode, ContainerNode } from '../types/editor';
import { isContainerNode } from './editor';

// Local type alias to avoid implicit any and reuse the union
...
export function detectFormatsInRange(
  ...
  let charsInRange = 0;
  let firstElementType: EditorElementType | undefined = undefined;
  let allSameElementType = true;
  let firstHref: string | null | undefined = undefined;
  let allSameHref = true;
  let firstClassName: string | null | undefined = undefined;
  let allSameClassName = true;
  let firstStyles: Record<string, string> | null | undefined = undefined;
  ...
  for (const child of node.children) {
    ...
    if (overlaps) {
      ...
      // Check element type (normalize 'p' to null as default block)
      const childElementType = child.elementType === "p" ? null : (child.elementType || null);
