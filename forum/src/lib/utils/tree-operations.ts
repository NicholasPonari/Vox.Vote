...
import { EditorNode, ContainerNode, StructuralNode } from "../types/editor";
import { isContainerNode, isStructuralNode } from "./editor";
...
export function updateNodeById(
  ...
  if (isContainerNode(node) || isStructuralNode(node)) {
    const containerNode = node as ContainerNode | StructuralNode;
    const newChildren = containerNode.children.map((child: EditorNode) =>
    ...
    const childrenChanged = newChildren.some(
      (newChild: EditorNode, index: number) => newChild !== containerNode.children[index]
    );
...
export function deleteNodeById(
  ...
  if (isContainerNode(node) || isStructuralNode(node)) {
    const containerNode = node as ContainerNode | StructuralNode;
    const newChildren = containerNode.children
      .map((child: EditorNode) => deleteNodeById(child, targetId))
      .filter((child: EditorNode | null): child is EditorNode => child !== null);

    // Only create new object if children changed
    if (newChildren.length !== containerNode.children.length) {
      return {
        ...node,
        children: newChildren,
      };
...
function insertNodeRecursive(
  ...
  // Check if target is a direct child
  const targetIndex = containerNode.children.findIndex((child: EditorNode) => child.id === targetId);
...
export function cloneNode(node: EditorNode, newId?: string): EditorNode {
  ...
  if ((isContainerNode(cloned) || isStructuralNode(cloned)) && (isContainerNode(node) || isStructuralNode(node))) {
    const containerNode = node as ContainerNode | StructuralNode;
    const containerCloned = cloned as ContainerNode | StructuralNode;
    containerCloned.children = containerNode.children.map((child: EditorNode) => cloneNode(child));
  }
