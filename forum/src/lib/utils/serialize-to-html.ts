...
import { ContainerNode, StructuralNode, TextNode } from '../types/editor';
import { isTextNode, isContainerNode, isStructuralNode, hasInlineChildren } from './editor';
  ...
  if (tag === 'table') {
    ...
    for (const child of node.children) {
      if (isStructuralNode(child)) {
        html += serializeTableNode(child as StructuralNode, indent + '  ');
      }
    }
...
function serializeContainerNode(node: ContainerNode, indent: string = ''): string {
  ...
  if (isTableWrapper && isStructuralNode(firstChild)) {
    ...
    return serializeTableNode(firstChild as StructuralNode, indent);
  }
