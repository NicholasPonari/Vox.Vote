...
import { TextNode, InlineText } from '../types/editor';
import { hasInlineChildren, getNodeTextContent } from './editor';
...
export function applyFormatting(
  ...
  if (before) {
    newChildren.push({ content: before });
  }
  
  // Add formatted selection as a span
  if (selected) {
    newChildren.push({ content: selected, className });
  }
  
  // Add "after" text if it exists
  if (after) {
    newChildren.push({ content: after });
  }
...
export function getFormattingAtPosition(node: TextNode, offset: number): string[] {
  ...
  for (const child of node.children!) {
    ...
    if (offset >= currentOffset && offset <= currentOffset + childLength) {
      return child.className ? [String(child.className)] : [];
    }
